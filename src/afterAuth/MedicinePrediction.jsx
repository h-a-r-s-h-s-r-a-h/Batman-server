...
const MedicineRecommendation = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const response = await fetch("http://127.0.0.1:8001/medicine_prediction", {
      method: "POST",
      headers: {
    ...
    console.log(data.M1[0].condition);
    toast.success("Your medicine is ready");
  };
  useEffect(() => {
    ...
    <div>
        ...
        <div className="container">
          <div className="row">
            <div className="col-lg-6 pt-5 pt-lg-0 order-2 order-lg-1 d-flex flex-column justify-content-center">
              <h1>MedicoMate's Medicine Predictor</h1>
              <h2>
                MedicoMate's Medicine Predictor employs AI to enhance treatment
                decisions, ensuring optimal outcomes.
              </h2>
              <div className="d-flex"></div>
            </div>
            <div className={`col-lg-6 order-1 order-lg-2 ${styles.heroimg}`}>
              <img src="/medicine100.png" class="img-fluid animated" alt="" />
            </div>
          </div>
        </div>
      </section>
      ...
      <div id="main">
        <section id="featured-services" className={styles.featuredservices8}>
          ...
          >
            <div>
              <h1>Predicted Medicines</h1>
              <ul class="list-group">
                {medicineData.M1[0] && (
                  <li class="list-group-item">
                    {medicineData.M1[0].drugName}{" "}
                    <button style={{ marginLeft: "350px" }}>Read More</button>
                  </li>
                )}
                {medicineData.M2[0] && (
                  <li class="list-group-item">
                    {medicineData.M2[0].drugName}{" "}
                    <button style={{ marginLeft: "350px" }}>Read More</button>
                  </li>
                )}
                {medicineData.M3[0] && (
                  <li class="list-group-item">
                    {medicineData.M3[0].drugName}{" "}
                    <Link to={`/MedicoDetails/${medicineData.M3[0].uniqueID}`}>
                      <button style={{ marginLeft: "350px" }}>Read More</button>
                    </Link>
                  </li>
                )}
          ...
          >
            <div>
              ...
              <ul className="list-group">
                {medicineData && medicineData.M1 && (
                  <li className="list-group-item">
                    {medicineData.M1[0]?.drugName}{" "}
                    <Link to={`/MedicoDetails/${medicineData.M1[0]?.uniqueID}`}>
                      <button style={{ marginLeft: "350px" }}>Read More</button>
                    </Link>
                  </li>
                )}
                {medicineData && medicineData.M2 && (
                  <li className="list-group-item">
                    {medicineData.M2[0]?.drugName}{" "}
                    <Link to={`/MedicoDetails/${medicineData.M2[0]?.uniqueID}`}>
                      <button style={{ marginLeft: "350px" }}>Read More</button>
                    </Link>
                  </li>
                )}
                {medicineData && medicineData.M3 && (
                  <li className="list-group-item">
                    {medicineData.M3[0]?.drugName}{" "}
                    <Link to={`/MedicoDetails/${medicineData.M3[0]?.uniqueID}`}>
                      <button style={{ marginLeft: "350px" }}>Read More</button>
